...
async def get_public_projects(
    ...
    order_by = {
        ...
        "data_desc": "p.published_at DESC NULLS LAST, p.id DESC",
    }.get(ordenacao, "p.published_at DESC NULLS LAST, p.id DESC")
...
async def get_public_project_by_id(conn: asyncpg.Connection, project_id: int) -> dict | None:
    row = await conn.fetchrow(
        ...
        SELECT p.id, p.sie_project_id, p.process_code, p.title, p.source_summary,
               COALESCE(p.local_short_description, p.source_summary) AS short_description, p.local_description AS full_description,
...
async def create_project_assignment(conn: asyncpg.Connection, project_id: int, user_id: int, user_role: str, descricao: str, course_ids: list[int]) -> dict:
    ...
    return {"has_project_access": True, "valid_course_count": valid_course_count, "requested_course_count": len(unique_course_ids), "assignment": {"atribuicao_id": row["id"], "projeto_id": row["project_id"], "descricao": row["description"], "curso_ids": unique_course_ids}}
