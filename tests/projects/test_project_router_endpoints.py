...
def test_get_my_project_detail_forwards_authenticated_user(monkeypatch: pytest.MonkeyPatch) -> None:
    conn = object()
    user = {"id": 99, "role": "tecnico"}
    service_mock = AsyncMock(return_value={"status": True, "message": "ok", "data": {"projeto": {"id": 10}}})
    monkeypatch.setattr(projects_router_module, "get_my_project_detail", service_mock)

    response = asyncio.run(projects_router_module.get_my_project_by_id(project_id=10, user=user, conn=conn))
...
def test_post_project_opportunity_returns_201_on_success(monkeypatch: pytest.MonkeyPatch) -> None:
    conn = object()
    user = {"id": 99, "role": "professor"}
    service_mock = AsyncMock(
        return_value={
            ...
            "data": {"opportunity": {"id": 8}},
        }
    )
    monkeypatch.setattr(projects_router_module, "create_my_project_opportunity", service_mock)
    payload = ProjectAssignmentCreateRequest(descricao="Descricao valida para oportunidade", curso_ids=[3, 2, 1])

    response = asyncio.run(
        projects_router_module.post_project_opportunity(project_id=10, payload=payload, user=user, conn=conn)
    )

    assert response.status_code == 201
    assert json.loads(response.body.decode("utf-8"))["data"]["opportunity"]["id"] == 8
    service_mock.assert_awaited_once_with(conn, user, 10, "Descricao valida para oportunidade", [1, 2, 3])


def test_delete_project_opportunity_returns_400_when_service_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    user = {"id": 99, "role": "professor"}
    monkeypatch.setattr(
        projects_router_module,
        "delete_my_project_opportunity",
        AsyncMock(return_value={"status": False, "message": "Oportunidade nao encontrada", "data": {}}),
    )

    response = asyncio.run(projects_router_module.delete_project_opportunity(opportunity_id=77, user=user, conn=object()))

    assert response.status_code == 400
    assert json.loads(response.body.decode("utf-8")) == {"detail": "Oportunidade nao encontrada"}
