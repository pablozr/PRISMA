...
DECLARE
  v_admin_id BIGINT;
  v_tecnico_id BIGINT;
  v_user_ana_id BIGINT;
  v_user_bruno_id BIGINT;
  v_user_carla_id BIGINT;
  v_batch_id BIGINT;
...
BEGIN
  ...
  FROM users
  WHERE institutional_email = 'ana.martins@demo.unirio.br';

  SELECT id INTO v_user_bruno_id
  FROM users
  WHERE institutional_email = 'bruno.costa@demo.unirio.br';

  SELECT id INTO v_user_carla_id
  FROM users
  WHERE institutional_email = 'carla.souza@demo.unirio.br';
  ...
  VALUES
    ('ana.martins@demo.unirio.br', 'Ana Martins', 'DEMO-SIAPE-001', v_dep_di_id, v_user_ana_id, v_batch_id, TRUE),
    ('bruno.costa@demo.unirio.br', 'Bruno Costa', 'DEMO-SIAPE-002', v_dep_bio_id, v_user_bruno_id, v_batch_id, TRUE),
    ('carla.souza@demo.unirio.br', 'Carla Souza', 'DEMO-SIAPE-003', v_dep_edu_id, v_user_carla_id, v_batch_id, TRUE)
  ON CONFLICT (institutional_email) DO UPDATE SET
    full_name = EXCLUDED.full_name,
  ...
  IF v_project_2 IS NULL THEN
    INSERT INTO projects (
      ...
      responsible_user_id, executing_unit_id, source_import_batch_id, project_type_id,
  ...
  ELSE
    UPDATE projects SET
      ...
      responsible_user_id = v_user_bruno_id,
